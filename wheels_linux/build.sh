...
build_core () {
    ...
    cmake --build "${KRATOS_ROOT}/build/Release" --target KratosKernel -- -j1
}
...
build_interface () {
    ...
    cmake --build "${KRATOS_ROOT}/build/Release" --target KratosPythonInterface -- -j1
    cmake --build "${KRATOS_ROOT}/build/Release" --target install -- -j1
}
